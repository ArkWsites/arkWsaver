(function () {
  ...
  const arkWsaver$2 = globalThis.arkWsaver;

  const SELECTED_CONTENT_ATTRIBUTE_NAME =
    arkWsaver$2.helper.SELECTED_CONTENT_ATTRIBUTE_NAME;

  const MASK_TAGNAME = "arkWsaver-mask";
  const MASK_CONTENT_CLASSNAME = "arkWsaver-mask-content";
  const PROGRESSBAR_CLASSNAME = "arkWsaver-progress-bar";
  const PROGRESSBAR_CONTENT_CLASSNAME = "arkWsaver-progress-bar-content";
  const SELECTION_ZONE_TAGNAME = "single-file-selection-zone";
  const LOGS_WINDOW_TAGNAME = "arkWsaver-logs-window";
  const LOGS_CLASSNAME = "arkWsaver-logs";
  const LOGS_LINE_CLASSNAME = "arkWsaver-logs-line";
  const LOGS_LINE_TEXT_ELEMENT_CLASSNAME = "arkWsaver-logs-line-text";
  const LOGS_LINE_STATUS_ELEMENT_CLASSNAME = "arkWsaver-logs-line-icon";
  const SINGLE_FILE_UI_ELEMENT_CLASS$1 =
    arkWsaver$2.helper.SINGLE_FILE_UI_ELEMENT_CLASS;
  ...
  const CSS_PROPERTIES$1 = new Set(
      ...
      if (range && range.commonAncestorContainer) {
        ...
        if (
          ...
          treeWalker.currentNode
            .querySelectorAll("*")
            .forEach((descendantElement) =>
              markSelectedNode(descendantElement)
            );
        }
      }
    }
  ...
  function createElement$1(tagName, parentElement) {
    ...
    CSS_PROPERTIES$1.forEach((property) =>
  ...
  const arkWsaver$1 = globalThis.arkWsaver;
  ...
  const SINGLE_FILE_UI_ELEMENT_CLASS =
    arkWsaver$1.helper.SINGLE_FILE_UI_ELEMENT_CLASS;
  const ERROR_BAR_TAGNAME = "arkWsaver-error-bar";
  ...
  function onError(message, link) {
    try {
      console.error("arkWsaver Lite", message, link); // eslint-disable-line no-console
      ...
      if (!errorBarElement) {
        ...
        errorTextElement.textContent = "arkWsaver Lite error: " + content[0];
  ...
  arkWsaver.init({ fetch: fetchResource, frameFetch });
  browser.runtime.onMessage.addListener((message) => {
    if (
        ...
        try {
          ...
          if (pageData) {
            if (
              ...
              pageData.filename =
                promptMessage("Save as", pageData.filename) ||
                pageData.filename;
            }
    ...
    if (!processor.cancelled) {
      if (options.confirmInfobarContent) {
        options.infobarContent =
          promptMessage("Infobar content", options.infobarContent) || "";
      }
