  ...
  class MediaUploader {
    ...
    async upload() {
      const httpResponse = getResponse(
        await fetch(GDRIVE_UPLOAD_URL + "?uploadType=resumable", {
          method: "POST",
          headers: {
    ...
    async function createContent({ path, content, message = "" }, signal) {
      try {
        const response = await fetch(
          `https://api.github.com/repos/${userName}/${repositoryName}/contents/${path}`,
          {
            method: "PUT",
            headers: new Map([
        ...
        await saveToGDrive(
          ...
          },
          {
            onProgress: (offset, size) =>
              onUploadProgress(tab.id, offset, size),
          }
        );
    ...
    try {
      ...
      if (!taskInfo || !taskInfo.cancelled) {
        return gDrive.upload(filename, blob, uploadOptions, (callback) =>
          setCancelCallback(taskId, callback)
        );
      }
    } catch (error) {
      if (error.message == "invalid_token") {
