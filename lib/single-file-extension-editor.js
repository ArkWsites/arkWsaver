(function () {
  ...
  ((globalThis) => {
    ...
    const DISABLED_NOSCRIPT_ATTRIBUTE_NAME =
      "data-single-file-disabled-noscript";
...
pre code {
  ...
  overflow: auto;
    ...
    window.onmessage = async (event) => {
      ...
      if (message.method == "getContent") {
        ...
        window.parent.postMessage(
          JSON.stringify({ method: "setContent", content }),
          "*"
        );
      }
    };
    ...
    async function init(content, { filename, reset } = {}) {
      await initConstants();
      const initScriptContentMatch = content.match(
        /<script data-template-shadow-root.*<\/script>/
      );
      ...
      content = content.replace(
        ...
        "<script data-template-shadow-root src=/lib/single-file-extension-editor-init.js></script>"
      );
      ...
      if (detectSavedPage(contentDocument)) {
        ...
        document.documentElement.appendChild(
          getStyleElement(HIGHLIGHTS_WEB_STYLESHEET)
        );
        ...
        document.documentElement.onmousedown =
          document.documentElement.ontouchstart = onMouseDown;
        document.documentElement.onmouseup =
          document.documentElement.ontouchend = onMouseUp;
    ...
    function formatPage(applySystemTheme) {
      ...
      document.querySelectorAll(NOTE_TAGNAME).forEach((containerElement) => {
        ...
        const className =
          "arkWsaver-note-id-" + containerElement.dataset.noteId;
    ...
    function getEmbedScript() {
      return minifyText(`(() => {
			...
			const maskPageElement = getMaskElement(${JSON.stringify(
        PAGE_MASK_CLASS
      )}, ${JSON.stringify(PAGE_MASK_CONTAINER_CLASS)});
      ...
      return (
        document.querySelector(
          "[data-single-file-note-refs^=" +
            JSON.stringify(containerElement.dataset.noteId) +
            "], [data-single-file-note-refs$=" +
            JSON.stringify(containerElement.dataset.noteId) +
            "], [data-single-file-note-refs*=" +
            JSON.stringify("," + containerElement.dataset.noteId + ",") +
    ...
    function detectSavedPage(document) {
      ...
      return (
        ...
        (firstDocumentChild.textContent.includes(COMMENT_HEADER) ||
          firstDocumentChild.textContent.includes(COMMENT_HEADER_LEGACY))
      );
    }
