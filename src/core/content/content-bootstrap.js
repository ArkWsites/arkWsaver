...
const arkWsaver = globalThis.arkWsaverBootstrap;
...
async function onMessage(message) {
  ...
  if (message.method == "devtools.resourceCommitted") {
    arkWsaver.pageInfo.updatedResources[message.url] = {
...
function init() {
  if (previousLocationHref != location.href && !arkWsaver.pageInfo.processing) {
    pageAutoSaved = false;
...
async function autoSavePage() {
  ...
  if ((!autoSavingPage || autoSaveTimeout) && !pageAutoSaved) {
    ...
    if (optionsAutoSave.autoSaveDelay && !autoSaveTimeout) {
      ...
      const waitForUserScript = window._arkWsaver_waitForUserScript;
      let frames = [];
      let framesSessionId;
      ...
      if (
        ...
        frames = await arkWsaver.processors.frameTree.getAsync(optionsAutoSave);
      }
      ...
      if (framesSessionId) {
        arkWsaver.processors.frameTree.cleanup(framesSessionId);
      }
...
function autoSaveUnloadedPage({
  ...
  if (
    ...
    frames = arkWsaver.processors.frameTree.getSync(optionsAutoSave);
  }
  if (optionsAutoSave.userScriptEnabled && waitForUserScript) {
...
function savePage(
  ...
  const helper = arkWsaver.helper;
  const updatedResources = arkWsaver.pageInfo.updatedResources;
  const visitDate = arkWsaver.pageInfo.visitDate.getTime();
...
async function openEditor(document) {
  const infobarElement = document.querySelector("arkWsaver-infobar");
  if (infobarElement) {
    infobarElement.remove();
  }
  serializeShadowRoots(document);
  const content = arkWsaver.helper.serialize(document);
  for (
    let blockIndex = 0;
...
function detectSavedPage(document) {
  ...
  return (
    ...
    (firstDocumentChild.textContent.includes(helper.COMMENT_HEADER) ||
      firstDocumentChild.textContent.includes(helper.COMMENT_HEADER_LEGACY))
  );
}
...
function serializeShadowRoots(node) {
  ...
  node.querySelectorAll("*").forEach((element) => {
    const shadowRoot = arkWsaver.helper.getShadowRoot(element);
    if (shadowRoot) {
